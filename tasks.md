...
## Phase 6: Viewport-First UI/UX リファクタ（見積: 3〜4h）✅ 完了

### 6.1 情報設計の再整理
- [x] 常時表示すべき情報と補助情報を再分類
- [x] コア戦場を中央、補助情報をサイドドックへ再配置
- [x] ログと星典プレビューをタブ切り替え式に整理

### 6.2 一画面HUDレイアウト
- [x] `ui.js` の画面構成を desktop-first に再編
- [x] CPUレーン / 中央ステージ / プレイヤーレーンの3段戦場を実装
- [x] 天窓と操作パネルを下部コマンド帯に再配置
- [x] 右ドック内で星典詳細とログを切り替えられるようにする

### 6.3 操作導線の改善
- [x] 現フェイズの主目的を中央ステージで常時表示
- [x] プレイヤーが次に押す場所を下部コマンド帯に集約
- [x] 星典プレビューを常設大パネルから戦略用サイドドックに移設

### 6.4 Viewport検証
- [x] `1366x768` でページ全体に縦スクロールが出ないことを確認
- [x] スタートフェイズ / 星典タブ / バトル中UI が一画面に収まることを確認
- [x] コンソールエラーなしを確認

---

## Phase 7: 内容拡張基盤 + 複数固定星典（見積: 5〜7h）✅ 完了

### 7.1 カード基盤の整理
- [x] `effectType + effectPayload` へ星命データを統一
- [x] `aiHints` と説明文をカード定義へ統合
- [x] カードプールを `星霊9 / 攻星術10 / 守星術6 / 星命6` に拡張

### 7.2 固定順星典プリセット
- [x] `CHRONICLE_PRESETS` を追加
- [x] `均衡の星典` / `焔風の星典` / `潮土の星典` の3系統を実装
- [x] 各プリセットに対して静的検証（1ページ目星霊 / 20ページ / 序盤コスト確認）を追加

### 7.3 ロジック・CPU整理
- [x] `initGame()` が `playerPresetId` / `cpuPresetId` を受け取れるようにする
- [x] 星命の効果解決を共通 resolver に整理
- [x] Fate 評価ロジックの重複を共通 helper に集約
- [x] CPU が `aiHints` と `effectPayload` を参照して攻防判断できるよう調整

### 7.4 開始前セットアップUI
- [x] タイトル面に軽量な対戦セットアップを追加
- [x] プレイヤー/CPU それぞれの固定順星典を選択可能にする
- [x] プリセット要約（系統、枚数、平均コスト）を画面内に表示

### 7.5 検証
- [x] `runPhase7Simulation()` を追加
- [x] Phase 2 / 4 / 5 / 7 のシミュレーションが通ることを確認
- [x] 3プリセット総当たりで勝率が `35%〜65%` に収まることを確認
- [x] `1366x768` でタイトル面と対戦HUDがスクロールなしで収まることを確認
- [x] コンソールエラーなしを確認
...
graph LR
    ...
    P3 --> P5[Phase 5: 統合・調整]
    P4 --> P5
    P5 --> P6[Phase 6: Viewport-First UI/UX]
    P6 --> P7[Phase 7: 内容拡張基盤]
```
...
| Phase 6 | Viewport-First UI/UX | 3〜4h | 一画面HUD、サイドドック、スクロールレス戦場 |
| Phase 7 | 内容拡張基盤 | 5〜7h | 複数固定星典、カード拡張、セットアップUI |
| **合計** | | **22〜31h** | |
